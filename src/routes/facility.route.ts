 ...
 */
router.post("/", hasRole(["super-admin"]), validate(createFacilitySchema), FacilityController.create);

/**
 * @route   PATCH /api/v1/facilities/:id
 * @desc    Update an existing facility's name, icon, or description
 */
router.patch("/:id", hasRole(["super-admin"]), validate(updateFacilitySchema), FacilityController.update);
